...
void sendMavlink()
{
	...
	if (t - lastFast >= PERIOD_FAST)
	{
		...
		if (channels[0] != 0)
			sendMessage(&msg); // 0 means no RC input
...
void handleMavlink(const void *_msg)
{
	...
	if (msg.msgid == MAVLINK_MSG_ID_MANUAL_CONTROL)
	{
		...
		if (m.target && m.target != SYSTEM_ID)
		...
		if (abs(controlYaw) < MAVLINK_CONTROL_YAW_DEAD_ZONE)
			controlYaw = 0;
	}
	...
	if (msg.msgid == MAVLINK_MSG_ID_PARAM_REQUEST_LIST)
	{
		...
		if (m.target_system && m.target_system != SYSTEM_ID)
			return;
	...
	if (msg.msgid == MAVLINK_MSG_ID_COMMAND_LONG)
	{
		...
		{
			if (m.param1 && controlThrottle > 0.05)
				return; // don't arm if throttle is not low
			accepted = true;
			armed = m.param1 == 1;
		}
